...
class lib_gui_Window(QMainWindow):
    ...
    def setCombo(self):
        self.lib_gui_ui.comboBox.addItems(["Вид животного", "Порода", "Клиент",
    ...
    def show_table(self):
        try:
            ...
            if self.table in "anim_type":
            ...
            elif self.table in "breed":
                row_labels = ["id", "Порода"]
                ...
                row_labels = ["id", "Дата", "Клиент", "Ветеринар", "Услуга", "Животное", "Было использовано", "Время приема", "Статус приема"]
            ...
            print('len(colum_name)', len(colum_name))
            print('colum_name', colum_name)

            cursor = self.conn.cursor()
            cursor.execute(f"SELECT * FROM anim_type")
            bib = cursor.fetchall()
